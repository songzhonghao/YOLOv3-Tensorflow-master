...
class YOLOV3(object):
    ...
    def __init__(self, input_data, trainable):
        ...
        self.strides          = np.array(cfg.YOLO.STRIDES)#strides已经修改
        self.anchors          = utils.get_anchors(cfg.YOLO.ANCHORS)#anchor数量已修改
        self.anchor_per_scale = cfg.YOLO.ANCHOR_PER_SCALE
        self.iou_loss_thresh  = cfg.YOLO.IOU_LOSS_THRESH
        self.upsample_method  = cfg.YOLO.UPSAMPLE_METHOD

        try:
            self.conv_lbbox, self.conv_mbbox, self.conv_sbbox, self.conv_sbbox_1 = self.__build_nework(input_data)
        except:
            raise NotImplementedError("Can not build up yolov3 network!")

        with tf.variable_scope('pred_sbbox_1'):
            self.pred_sbbox_1 = self.decode(self.conv_sbbox_1, self.anchors[0], self.strides[0])

        with tf.variable_scope('pred_sbbox'):
            self.pred_sbbox = self.decode(self.conv_sbbox, self.anchors[1], self.strides[1])

        with tf.variable_scope('pred_mbbox'):
            self.pred_mbbox = self.decode(self.conv_mbbox, self.anchors[2], self.strides[2])

        with tf.variable_scope('pred_lbbox'):
            self.pred_lbbox = self.decode(self.conv_lbbox, self.anchors[3], self.strides[3])
    ...
    def compute_loss(self, label_sbbox_1, label_sbbox, label_mbbox, label_lbbox, true_sbbox_1, true_sbbox, true_mbbox, true_lbbox):

        with tf.name_scope('smaller_box_loss_1'):
            loss_sbbox_1 = self.loss_layer(self.conv_sbbox_1, self.pred_sbbox_1, label_sbbox_1, true_sbbox_1,
        ...
        with tf.name_scope('bigger_box_loss'):
            loss_lbbox = self.loss_layer(self.conv_lbbox, self.pred_lbbox, label_lbbox, true_lbbox,
                                         anchors = self.anchors[3], stride = self.strides[3])

        with tf.name_scope('giou_loss'):
            giou_loss = loss_sbbox_1[0] + loss_sbbox[0] + loss_mbbox[0] + loss_lbbox[0]

        with tf.name_scope('conf_loss'):
            conf_loss = loss_sbbox_1[1] + loss_sbbox[1] + loss_mbbox[1] + loss_lbbox[1]

        with tf.name_scope('prob_loss'):
            prob_loss = loss_sbbox_1[2] + loss_sbbox[2] + loss_mbbox[2] + loss_lbbox[2]
